...
public class _FinalVisitor2 extends ExprBaseVisitor<Object> {
	...
	public Object visitExpr(ExprContext ctx) {
		...
		BigDecimal a = (BigDecimal) visitExpr1(ctx.expr1(0));
		for (int i = 1; i < ctx.getChildCount(); i += 2) {
			String op = ctx.getChild(i).getText();
			BigDecimal b = (BigDecimal) visitExpr1((Expr1Context) ctx.getChild(i + 1));
			switch (op) {
			case "+":
				a = a.add(b);
				break;
	...
	public Object visitExpr1(Expr1Context ctx) {
		...
		BigDecimal a = (BigDecimal) visitExpr0(ctx.expr0(0));
		for (int i = 1; i < ctx.getChildCount(); i += 2) {
			String op = ctx.getChild(i).getText();
			BigDecimal b = (BigDecimal) visitExpr0((Expr0Context) ctx.getChild(i + 1));
			switch (op) {
			case "*":
				a = a.multiply(b);
				break;
	...
	@Override
	public Object visitExpr0(Expr0Context ctx) {
		String tabs = tabs(ctx.depth());
		LOGGER.debug("{}数字或括号:: {}", tabs, ctx.getText());
		Object ret = ctx.expr() == null ? new BigDecimal(ctx.getText()) : visitExpr(ctx.expr());
